"""Real-LLM diagnostic for model strength and structured sampling on GSM8K.

Scientific intent:
1. Check whether the current model is too weak for reasoning-mode extra compute
   to matter.
2. Check whether simple structured sampling is more useful than naive repeated
   sampling.
3. Check whether adaptive-compute questions become more meaningful once we use
   a stronger model and a better small-budget sampling strategy.
"""
...
PROMPT_REASONING = (
    "Solve the following question carefully step by step. "
    "At the end, state the final numeric answer clearly."
)
PROMPT_STRUCTURED_CONCISE = (
    "Attempt 1 of 3. Solve the following question with a concise direct answer. "
    "Give only the final numeric answer."
)
PROMPT_STRUCTURED_REASONING = (
    "Attempt 2 of 3. Solve the following question carefully step by step. "
    "Conclude with: Final answer: <number>."
)
PROMPT_STRUCTURED_DOUBLE_CHECK = (
    "Attempt 3 of 3. Solve the following question, then explicitly double-check "
    "the result before giving the verified final numeric answer."
)

SCIENTIFIC_INTENT = (
    "Diagnostic run for whether the current model is too weak for reasoning-mode "
    "compute to matter, whether structured sampling beats naive repeated "
    "sampling, and whether adaptive-compute questions become more meaningful "
    "with a stronger model and better sampling."
)
MAX_QUERY_LIMIT = 30
...
def _run_strategy_for_query(
    ...
    for label, prompt_prefix in strategy.prompt_labels_and_prefixes:
        ...
        parsed_candidates.extend(
            extract_numeric_answer(response) for response in raw_responses
        )
    ...
    return {
        ...
        "aggregation_method": "normalized_numeric_majority_vote",
...
def build_comparison_summary(
    ...
    for model_name in tested_models:
        ...
        per_model_improvements.append(
            {
                "model": model_name,
                "reasoning_best_of_3_minus_reasoning_greedy": round(
                    float(reasoning_best_of_3["accuracy"])
                    - float(reasoning_greedy["accuracy"]),
                    4,
                ),
                "structured_sampling_3_minus_reasoning_best_of_3": round(
                    float(structured_sampling_3["accuracy"])
                    - float(reasoning_best_of_3["accuracy"]),
                    4,
                ),
            }
...
def run_model_sampling_diagnostic(config: dict[str, Any]) -> dict[str, Any]:
    """Run the diagnostic using the configured current model and stronger model."""
    ...
    current_rows, current_error = _run_model(current_model, queries, config)
